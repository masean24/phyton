...
MIN_FREE_RAM_MB = 500  # 1.5 GB minimum
...
def kill_zombie_browsers() -> int:
    """Kill semua proses brave/chrome/chromedriver yang orphan/zombie."""
    killed = 0
    try:
        for proc in psutil.process_iter(['pid', 'name', 'status']):
            try:
                name = (proc.info['name'] or '').lower()
                if any(x in name for x in ['brave', 'chrome', 'chromedriver']):
                    proc.kill()
                    killed += 1
...
def wait_for_ram(tag: str, timeout: int = 120) -> bool:
    ...
    # Coba kill zombie browser dulu
    killed = kill_zombie_browsers()
    if killed:
        print(f"{tag} 🗑️ Killed {killed} zombie browser process(es)")
        time.sleep(3)
        if check_ram_available():
            return True
    ...
    while time.time() - start < timeout:
        ...
        # Setiap 30 detik, coba kill lagi
        elapsed = int(time.time() - start)
        if elapsed > 0 and elapsed % 30 == 0:
            k = kill_zombie_browsers()
            if k:
                print(f"{tag} 🗑️ Killed {k} zombie process(es) (retry)")
...
# ═══════════════════════════════════════════════════════════════
#  MAINTENANCE — auto-cleanup logs, screenshots, temp folders
# ═══════════════════════════════════════════════════════════════

MAX_LOG_SIZE_MB = 10          # Max error log size
MAX_SCREENSHOT_AGE_HOURS = 1  # Hapus screenshot lebih lama dari ini


def rotate_error_log() -> None:
    """Kalau errors.txt > MAX_LOG_SIZE_MB, rename ke .old dan mulai baru."""
    try:
        if not os.path.isfile(ERROR_LOG):
            return
        size_mb = os.path.getsize(ERROR_LOG) / (1024 * 1024)
        if size_mb >= MAX_LOG_SIZE_MB:
            old_log = ERROR_LOG + ".old"
            # Hapus .old sebelumnya kalau ada
            if os.path.isfile(old_log):
                os.remove(old_log)
            os.rename(ERROR_LOG, old_log)
            print(f"📄 Error log dirotasi ({size_mb:.1f} MB → errors.txt.old)")
    except Exception:
        pass


def cleanup_old_screenshots() -> None:
    """Hapus screenshot .png yang lebih dari MAX_SCREENSHOT_AGE_HOURS."""
    try:
        now = time.time()
        cutoff = now - (MAX_SCREENSHOT_AGE_HOURS * 3600)
        deleted = 0
        for fname in os.listdir(LOGS_DIR):
            ...
            try:
                ...
                continue
        if deleted:
            print(f"🗑️ {deleted} screenshot lama dihapus (> {MAX_SCREENSHOT_AGE_HOURS} jam)")
    except Exception:
        pass


def cleanup_stale_temp() -> None:
    """Hapus temp profile folders yang masih tersisa (gagal cleanup)."""
    try:
        ...
        deleted = 0
        for dirname in os.listdir(PROFILES_DIR):
            dpath = os.path.join(PROFILES_DIR, dirname)
            ...
            try:
                ...
                continue
        if deleted:
            print(f"🗑️ {deleted} stale temp profile dihapus")
    except Exception:
        pass


def maintenance_cleanup() -> None:
    """Jalankan semua maintenance tasks. Dipanggil saat cooldown."""
    rotate_error_log()
    cleanup_old_screenshots()
    cleanup_stale_temp()
    # Kill zombie browser yang mungkin tertinggal
    killed = kill_zombie_browsers()
    if killed:
        print(f"🗑️ Killed {killed} zombie browser process(es)")
...
class ProxyConfig:
    ...
    def display(self) -> str:
        ...
        return f"{self.host}:{self.port} (no auth)"
...
def parse_proxy(line: str) -> Optional[ProxyConfig]:
    ...
    Parse proxy string format:
      ...
      - host:port:username:password        (dengan auth)
    Contoh:
      65.111.11.2:3129
      growtechcentral.com:10000:ca985b86519d8d3b5068__cr.id:0324ca8617654dad
    """
    line = line.strip()
    ...
    if not host or not port:
        return None
    # Format tanpa auth: host:port
    if len(parts) == 2:
        return ProxyConfig(host=host, port=port)
    # Format dengan auth: host:port:username:password
    if len(parts) >= 4:
        username = parts[2]
...
def create_proxy_auth_extension(proxy: ProxyConfig, thread_id: int = 0) -> str:
    ...
    # Auth handler hanya ditambahkan kalau proxy butuh auth
    auth_block = ""
    if proxy.needs_auth:
        auth_block = f"""
...
// ── Anti Arkose Labs / FunCaptcha detection ──

// Fix: chrome.runtime.connect should not throw 
if (window.chrome) {
  const origConnect = window.chrome.runtime?.connect;
  window.chrome.runtime = window.chrome.runtime || {};
  window.chrome.runtime.connect = function() {
    try { return origConnect?.apply(this, arguments); } catch(e) { return {}; }
  };
...
// Prevent detection via Error stack traces (Arkose checks for "selenium" / "webdriver" in stack)
...
// Override performance timing (Arkose checks for suspicious timing patterns)
const origNow = Performance.prototype.now;
let perfOffset = Math.random() * 0.1;
Performance.prototype.now = function() {
  return origNow.call(this) + perfOffset;
};

// Fix: document.hasFocus should return true
...
def find_browser_path() -> tuple[str, str]:
    """Return (path, name) — Brave prioritized over Chrome. Support Windows & Linux."""
    ...
    if is_linux:
        browsers = [
            ("Brave", [
                ...
                "/snap/bin/brave",
                os.path.join(home, ".local/bin/brave-browser"),
            ]),
            ("Chrome", [
...
# ═══════════════════════════════════════════════════════════════
#  COOKIE BANNER DISMISSAL (OneTrust / ot-sdk / Cookiebot)
# ═══════════════════════════════════════════════════════════════

def dismiss_cookie_banner(driver, tag: str = "", restore_wait: int = 5) -> bool:
    """
    Hapus cookie consent banner yang menghalangi klik.
    Dipanggil setelah setiap navigasi page baru.
    Return True jika banner ditemukan & dihapus.
    """
    try:
        old_wait = restore_wait
        driver.implicitly_wait(0)
        dismissed = False

        # Metode 1: Klik tombol Accept/Close di OneTrust
        ...
        for csel in cookie_btn_selectors:
            try:
                btns = driver.find_elements(By.CSS_SELECTOR, csel)
                for btn in btns:
                    if btn.is_displayed():
                        driver.execute_script("arguments[0].click();", btn)
                        dismissed = True
                        if tag:
                            print(f"{tag} 🍪 Cookie banner dismissed ({csel})")
                        break
            except Exception:
                pass
            if dismissed:
        ...
        # Metode 2: Hapus semua cookie banner dari DOM
        removed = driver.execute_script("""
            ...
            return count;
        """)
        if removed and not dismissed:
            dismissed = True
            if tag:
                print(f"{tag} 🍪 Cookie banner removed from DOM ({removed} elements)")
...
def run_bot(
    ...
    try:
        ...
        # ── Extra anti-detection args (bypass Arkose Labs / reCAPTCHA Enterprise) ──
        ...
        # Extra CDP stealth: override webdriver detection
        driver.execute_cdp_cmd("Page.addScriptToEvaluateOnNewDocument", {"source": """
            // Remove webdriver property traces
            delete Object.getPrototypeOf(navigator).webdriver;
            // Override toString leaks from chrome runtime
            if (window.chrome && window.chrome.runtime) {
                window.chrome.runtime.id = undefined;
            }
            // Spoof iframe contentWindow detection
        ...
        # ═══════════════════════════════════════════════════════
        #  STEP 1 — Buka Coursera + langsung trigger signup popup
        #  URL dengan ?action=enroll&authMode=login otomatis
        #  buka popup pendaftaran (skip klik "Enroll for free")
        # ═══════════════════════════════════════════════════════
        print(f"{tag} 🌐 Step 1: Membuka Coursera + auto enroll popup...")
        try:
            driver.get("https://www.coursera.org/google-specializations/ai-essentials-gwg?action=enroll&authMode=login")
            random_delay(3000, 5000)
        except Exception as e:
            print(f"{tag} ⚠️ Specialization gagal: {e}, fallback ke course...")
            try:
                driver.get("https://www.coursera.org/learn/google-ai-essentials?action=enroll&authMode=login")
                random_delay(3000, 5000)
            except Exception as e2:
                save_screenshot(driver, tag, "err-load")
                raise e2

        print(f"{tag} ℹ️ Step 2-3 dilewati (popup signup otomatis muncul via URL param)")
        random_delay(2000, 4000)

        # ── Dismiss cookie consent banner ──
        dismiss_cookie_banner(driver, tag)

        # ── Handle GDPR consent wall (muncul di beberapa IP luar) ──
        # Halaman GDPR punya checkbox + tombol "Continue" dengan
        # data-track-app="gdpr" data-track-page="wall"
        try:
            driver.implicitly_wait(0)
            gdpr_btns = driver.find_elements(
                By.CSS_SELECTOR,
                "button[data-track-app='gdpr'][data-track-page='wall']"
            )
            if gdpr_btns:
                print(f"{tag} 🌍 GDPR wall terdeteksi, handle consent...")

                # 1) Centang semua checkbox di halaman GDPR
                gdpr_checkboxes = driver.find_elements(
                    By.CSS_SELECTOR, "input[type='checkbox']"
                )
                for cb in gdpr_checkboxes:
                    try:
                        if not cb.is_selected():
                            ...
                            print(f"{tag} ✅ GDPR checkbox tercentang")
                    except Exception:
                        pass

                # Kadang checkbox pakai label klik — coba juga klik <label>
                if not gdpr_checkboxes:
                    gdpr_labels = driver.find_elements(
                        By.CSS_SELECTOR,
                        "label[for], .cds-checkboxLabel, [class*='checkbox']"
                    )
                    for lbl in gdpr_labels:
                        try:
                            if lbl.is_displayed():
                                lbl.click()
                                random_delay(300, 500)
                                print(f"{tag} ✅ GDPR label clicked")
                                break
                        except Exception:
                            continue
                ...
                # 2) Klik tombol "Continue" GDPR
                for gb in gdpr_btns:
                    try:
                        if gb.is_displayed() and gb.is_enabled():
                            driver.execute_script(
                                "arguments[0].scrollIntoView({block:'center'});", gb)
                            random_delay(500, 1000)
                            try:
                                gb.click()
                            except (ElementClickInterceptedException,
                            ...
                            print(f"{tag} ✅ GDPR Continue diklik")
                            break
                    except Exception:
                        continue

                random_delay(3000, 5000)
                print(f"{tag} ✅ GDPR wall selesai, lanjut ke signup...")

                # Dismiss cookie banner lagi (bisa muncul setelah GDPR)
                dismiss_cookie_banner(driver, tag)
            else:
                print(f"{tag} ℹ️ Tidak ada GDPR wall, lanjut...")
            driver.implicitly_wait(5)
        except Exception as e:
            print(f"{tag} ⚠️ GDPR check error: {e}")
            driver.implicitly_wait(5)
        ...
        # Simulasi mouse movement ke field sebelum klik (human-like)
        actions = ActionChains(driver)
        actions.move_to_element(email_input).pause(random.uniform(0.3, 0.7)).click().perform()
        random_delay(300, 600)
        email_input.clear()

        # Ketik email satu-satu (human-like) agar reCAPTCHA tidak curiga
        ...
        dismiss_cookie_banner(driver, tag)  # banner bisa muncul ulang
        random_delay(500, 1000)
        try:
            actions = ActionChains(driver)
            actions.move_to_element(continue_btn).pause(random.uniform(0.3, 0.6)).click().perform()
        ...
        # Human-like: mouse move ke name field → klik → ketik pelan
        actions = ActionChains(driver)
        actions.move_to_element(name_input).pause(random.uniform(0.3, 0.7)).click().perform()
        random_delay(300, 600)
        ...
        # Human-like: mouse move ke password field → klik → ketik pelan
        actions = ActionChains(driver)
        actions.move_to_element(pass_input).pause(random.uniform(0.3, 0.7)).click().perform()
        random_delay(300, 600)
        human_type(pass_input, password)
        random_delay(1000, 2000)

        # ── Tunggu reCAPTCHA Enterprise selesai evaluate (PENTING!) ──
        # Arkose Labs / reCAPTCHA butuh waktu ~3-5 detik setelah form diisi
        # untuk menentukan apakah user asli atau bot
        print(f"{tag} ⏳ Menunggu reCAPTCHA evaluate...")
        random_delay(3000, 5000)

        # "Join for Free" — cari yang visible
        dismiss_cookie_banner(driver, tag)  # banner bisa muncul ulang

        # Coba beberapa selector untuk tombol Join
        join_selectors = [
            "//button[contains(text(),'Join for Free')]",
            "//button[contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'join for free')]",
            "//button[@type='submit']",
            "//button[contains(@data-e2e, 'signup')]",
            "//button[contains(@class, 'signup')]",
        ]

        join_clicked = False
        for selector in join_selectors:
            join_btns = driver.find_elements(By.XPATH, selector)
            for btn in join_btns:
                if btn.is_displayed():
                    driver.execute_script("arguments[0].scrollIntoView({block:'center'});", btn)
                    random_delay(500, 1000)
                    try:
                        # Human-like: move mouse ke button lalu klik
                        actions = ActionChains(driver)
                        actions.move_to_element(btn).pause(random.uniform(0.4, 0.8)).click().perform()
                    except (ElementClickInterceptedException, Exception):
                        try:
                            btn.click()
                        except (ElementClickInterceptedException, Exception):
        ...
        if not join_clicked:
            # Last resort: cari tombol submit apapun di form
            print(f"{tag} ⚠️ Join button tidak ditemukan via XPath, coba CSS submit...")
            try:
                submit_btn = driver.find_element(By.CSS_SELECTOR, "form button[type='submit'], button[type='submit']")
                driver.execute_script("arguments[0].scrollIntoView({block:'center'});", submit_btn)
                random_delay(500, 800)
                actions = ActionChains(driver)
                actions.move_to_element(submit_btn).pause(random.uniform(0.3, 0.6)).click().perform()
                join_clicked = True
                print(f"{tag} ✅ Klik submit button (fallback)")
            except Exception as e:
                print(f"{tag} ❌ Tidak bisa klik Join for Free: {e}")
                save_screenshot(driver, tag, "err-join-btn")
        ...
        for poll in range(38):  # 38 x 2s = 76 detik max
            ...
            # --- Cek & dismiss GDPR wall (bisa muncul lagi setelah signup) ---
            try:
                gdpr_btns_loop = driver.find_elements(
                    By.CSS_SELECTOR,
                    "button[data-track-app='gdpr'][data-track-page='wall']"
                )
                if gdpr_btns_loop:
                    print(f"{tag} 🌍 GDPR wall terdeteksi (post-signup), handle consent...")
                    # Centang semua checkbox
                    gdpr_cbs = driver.find_elements(By.CSS_SELECTOR, "input[type='checkbox']")
                    for cb in gdpr_cbs:
                        try:
                            if not cb.is_selected():
                                try:
                            ...
                            pass
                    # Fallback: klik label kalau checkbox tidak ada
                    if not gdpr_cbs:
                        for lbl in driver.find_elements(By.CSS_SELECTOR, "label[for], .cds-checkboxLabel, [class*='checkbox']"):
                            try:
                                if lbl.is_displayed():
                                    lbl.click()
                                    random_delay(300, 500)
                        ...
                        try:
                            if gb.is_displayed() and gb.is_enabled():
                                try:
                                    ...
                                    driver.execute_script("arguments[0].click();", gb)
                                print(f"{tag} ✅ GDPR wall (post-signup) dismissed")
                                break
                        except Exception:
                            continue
                    random_delay(2000, 3000)
                    dismiss_cookie_banner(driver, tag, restore_wait=0)
                    continue  # ulang polling dari awal setelah GDPR
            except Exception:
                pass

            # --- Dismiss cookie banner kalau muncul ---
            try:
                dismiss_cookie_banner(driver, tag, restore_wait=0)
            except Exception:
                pass
        ...
        # ── Dismiss cookie banner (muncul lagi di checkout page) ──
        dismiss_cookie_banner(driver, tag)
        ...
        # ── Dismiss cookie banner sekali lagi sebelum interact Stripe ──
        dismiss_cookie_banner(driver, tag)
        ...
        # Cleanup — agresif kill semua child process browser
        if driver:
            try:
                # Kill semua child process browser (mencegah zombie)
                try:
...
def main() -> None:
    ...
    if ip_choice == "1":
        ...
        if not proxy_list:
            ...
            print("   Format per baris:")
            print("   - host:port                   (tanpa auth)")
            print("   - host:port:username:password  (dengan auth)")
            print(f"   Contoh: 65.111.11.2:3129")
        ...
        # ── Single proxy ────────────────────────────────────
        proxy_str = input("Proxy (host:port atau host:port:user:pass): ").strip()
        single_proxy = parse_proxy(proxy_str)
        if not single_proxy:
            print("❌ Format proxy tidak valid!")
            print("   Gunakan: host:port  atau  host:port:username:password")
            sys.exit(1)
    ...
    accounts_since_rest = 0       # counter untuk cooldown setiap 15 akun
    consecutive_fails = 0         # counter gagal berturut-turut

    COOLDOWN_EVERY = 10           # istirahat setiap 10 akun
    COOLDOWN_FAIL_THRESHOLD = 5   # istirahat setelah 5 gagal berturut-turut
    COOLDOWN_MIN = 5 * 60         # 5 menit (detik)
    COOLDOWN_MAX = 10 * 60        # 10 menit (detik)

    def do_cooldown(reason: str) -> None:
        """Istirahat 5-10 menit acak + jalankan maintenance."""
        nonlocal accounts_since_rest, consecutive_fails
        duration = random.randint(COOLDOWN_MIN, COOLDOWN_MAX)
        mins = duration / 60
        print(f"\n😴 COOLDOWN — {reason}")
        print(f"   Istirahat {mins:.1f} menit...")
        print(f"   Progress saat ini: {completed}/{total} | ✅ {success} | ❌ {failed}")

        # Jalankan maintenance saat cooldown
        print("   🔧 Menjalankan maintenance...")
        maintenance_cleanup()

        time.sleep(duration)
        accounts_since_rest = 0
        consecutive_fails = 0
        print(f"⏰ Cooldown selesai, lanjut proses...\n")
    ...
    while queue:
        ...
        # ── Cooldown check ────────────────────────────────
        if queue:
            if consecutive_fails >= COOLDOWN_FAIL_THRESHOLD:
                do_cooldown(f"Sudah {consecutive_fails} gagal berturut-turut")
            elif accounts_since_rest >= COOLDOWN_EVERY:
                do_cooldown(f"Sudah {accounts_since_rest} akun diproses")
